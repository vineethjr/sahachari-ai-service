...
async def llm_intent_fallback(query: str, pipe) -> str | None:
    ...
    try:
        ...
        def run():
            out = pipe(messages, max_new_tokens=8, return_full_text=False, do_sample=False)
            return out[0]["generated_text"]

        raw = await loop.run_in_executor(None, run)
        label = raw.strip().lower().split()[0].strip(".,:;\"'").replace("`", "")
        if label in INTENT_LABELS:
            return label
        for cand in INTENT_LABELS:
            if cand in raw.lower():
                return cand
    except Exception as e:
        log.warning(f"llm_intent_fallback failed: {e}")
    return None
...
def parse_quantity_reply(query: str):
    q = query.lower().strip()
    # FIX: was re.match with ^ anchor — too strict, rejected "1 kg please" etc.
    # Now uses re.search so partial matches like "about 500g" also work.
    pattern = rf"{NUM_WORDS_RE}\s*(?:{UNIT_WORDS_RE})?"
    m = re.search(pattern, q)
    if not m:
        return None, None
    raw_qty  = m.group(1)
    raw_unit = m.group(2)
    quantity = float(raw_qty) if raw_qty.replace(".", "", 1).isdigit() else float(WORD_TO_NUM.get(raw_qty, 1.0))
...
def extract_grocery_items(query: str) -> list:
    q = query.lower().strip()
    q = re.sub(
        r"^(add|get|buy|order|give me|i want|i need|i would like|want to buy|want to order)\s+", "", q
    )
    q = q.replace(",", " and ")
    ...
    for segment in segments:
        ...
        for m in matches:
            ...
            if not item_words:
                continue
            clean_item = " ".join(item_words)
            quantity   = float(raw_qty) if raw_qty.replace(".", "", 1).isdigit() else float(WORD_TO_NUM.get(raw_qty, 1.0))
            unit       = UNIT_NORMALISE.get(raw_unit, "piece") if raw_unit else "piece"
            items.append({"item": clean_item.title(), "quantity": quantity, "unit": unit})
...
def load_chroma_collection():
    ...
    try:
        ...
        log.error(
            ...
            f"Run ingest.py from {BASE_DIR} to rebuild it. RAG disabled until then."
        )
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        log.error(f"MongoDB FAILED: {type(e).__name__}: {e}. Check MONGO_URI in .env.")
